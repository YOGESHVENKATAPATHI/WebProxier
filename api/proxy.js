...
export default async function handler(req, res) {
  ...
  let targetUrlObj;
  try {
    // Trim any leading/trailing whitespace and quotes from the URL
    let trimmedUrl = url.trim().replace(/^["']|["']$/g, '').trim();
    
    // Fix common copy-paste errors like "https:// https://website.com"
    trimmedUrl = trimmedUrl.replace(/^(https?:\/\/)\s*(https?:\/\/)+/i, '$1');
    
    const urlString = trimmedUrl.startsWith('http') ? trimmedUrl : `https://${trimmedUrl}`;
    ...
    console.error('URL parsing error:', e.message, 'for URL:', url);
